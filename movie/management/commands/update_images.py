...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        # ✅ Load environment variables from moviereviews/openAI.env (same dir as manage.py)
        load_dotenv('openAI.env')

        # ✅ Resolve API key (supports OPENAI_API_KEY or openai_apikey)
        api_key = os.environ.get('OPENAI_API_KEY') or os.environ.get('openai_apikey')
        if not api_key:
            self.stderr.write(self.style.ERROR(
                "OpenAI API key not found. Add OPENAI_API_KEY to moviereviews/openAI.env or set it in your environment."
            ))
            return
